...
export function migrateV1Ledger(dataDir?: string): MigrationResult {
  const dir = dataDir ?? defaultDataDir();
  const v1Path = join(dir, 'ledger.json');
  const backupPath = join(dir, 'ledger.v1.json.bak');
  if (!existsSync(v1Path)) return { performed: false };
  try {
    renameSync(v1Path, backupPath);
    const warning =
      `[AsyncThink] Migrated v1 ledger to ${backupPath}. ` +
    ...
    return {
      performed: false,
      warning: `[AsyncThink] Failed to back up v1 ledger at ${v1Path}: ${(err as Error).message}`,
    };
  }
}
