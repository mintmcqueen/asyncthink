...
export function registerAsyncThinkTool(server) {
    server.registerTool('asyncthink', {
        title: 'AsyncThink',
        ...
        if (!activeChainId)
            activeChainId = council.newChain();
        ...
        if (args.forks) {
            for (const f of args.forks) {
                try {
                    ...
                    if (!adapter) {
                        throw new Error(`fork "${f.id}": either adapter or skill must be supplied (or set defaults.adapter via asyncthink_config).`);
                    }
        ...
        for (const id of args.waitFor ?? []) {
            if (researchResults.some((r) => r.id === id))
                continue;
        ...
        if (endResults) {
            for (const r of endResults) {
                if (!researchResults.some((x) => x.id === r.id))
                    researchResults.push(r);
            }
        }
        ...
        if (status.complete.length > 0) {
            reminderParts.push(`Forks complete: ${status.complete.join(', ')}. Use readResearch to inject results.`);
        }
        ...
        if (spawnErrors.length > 0) {
            reminderParts.push(`Spawn errors: ${spawnErrors.map((e) => `${e.id}: ${e.error}`).join('; ')}`);
        }
