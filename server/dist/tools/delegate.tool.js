 ...
 */
import { z } from 'zod';
import { getDelegate, getThreadStore, getSkillRegistry, getManifestRegistry, getAuditLog, } from '../app.js';
...
export function registerDelegateTools(server) {
    server.registerTool('delegate', {
        ...
        inputSchema: {
            ...
            skill: z
                .string()
                .optional()
                .describe("Skill id from the registry. The skill's frontmatter supplies the adapter and a prompt prefix; caller may still override model/timeout."),
            close: z
                .boolean()
                .optional()
            ...
            intelligence: z
                .enum(['high', 'med', 'low'])
                .optional()
                .describe('Intelligence tier — preferred over raw model id. Maps to a model via the adapter manifest, so callers stay stable as model names evolve.'),
            model: z
                .string()
                .optional()
            ...
            async: z
                .boolean()
                .optional()
                .describe('v2.2 — when true, spawn the work in the background and return {taskId}. Poll via tasks_get/tasks_result; cancel via tasks_cancel.'),
            idempotencyKey: z
                .string()
                .optional()
                .describe('v2.2 (async only) — caller-supplied dedup key. A repeat call with the same key + principal returns the existing in-flight taskId instead of spawning a new one.'),
            ttlMs: z
                .number()
                .int()
                .min(60_000)
            ...
            mcpServers: z
                .array(z.string())
                .optional()
                .describe('v2.3 — additive MCP-server allowlist for the adapter subprocess (F3-D.2). Merged with manifest defaults and any skill-supplied list.'),
            preflight: z
                .enum(['auth', 'none'])
                .optional()
    ...
    server.registerTool('delegate_close', {
        title: 'Delegate Close',
        description: 'Close a single delegate thread. Idempotent — closing an already-closed or unknown thread is a no-op.',
        inputSchema: {
    ...
    server.registerTool('delegate_close_all', {
        title: 'Delegate Close All',
        description: 'Close every open delegate thread. End-of-session safety net — call when wrapping up if any threads might be leaking.',
        inputSchema: {},
    }, async () => {
        await sweepIdleOnce();
    ...
    server.registerTool('delegate_list_threads', {
