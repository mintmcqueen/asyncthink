...
export function registerTasksTools(server) {
    server.registerTool('tasks_get', {
        title: 'Tasks Get',
        description: 'Snapshot the current status of an async task created by delegate({async:true}) or council forks. Returns status (working|completed|failed|cancelled), createdAt, lastUpdatedAt, ttlMs, and any current error. Idempotent.',
        inputSchema: {
    ...
    server.registerTool('tasks_list', {
        title: 'Tasks List',
        description: 'List async tasks with cursor pagination. v2.2 single-tenant: returns every task owned by `principal: null`. Use `cursor` from a prior response to fetch the next page; default page size is 50.',
        inputSchema: {
    ...
    server.registerTool('tasks_cancel', {
        title: 'Tasks Cancel',
        description: 'Best-effort cancel an in-flight task. Flips state to "cancelled" immediately and signals SIGTERM to the underlying subprocess (R-DUR-D.5). Idempotent: cancelling a terminal task is a no-op that returns its current state.',
        inputSchema: {
            taskId: z.string(),
        },
    }, async (args) => {
    ...
    server.registerTool('tasks_result', {
        title: 'Tasks Result',
        description: 'Block until the task reaches a terminal state, then return its full state including result text, exitCode, durationMs, and continuation sessionId. Cancelled or failed tasks return a state with `error` populated. Use tasks_get if you need a non-blocking snapshot.',
        inputSchema: {
            taskId: z.string(),
        },
    }, async (args) => {
