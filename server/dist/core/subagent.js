 ...
 * read-only" invariant.
 */
export const SECURITY_REVIEW_SUBAGENT = {
    id: 'security-review',
    name: 'Security Review',
    description: 'Adversarial security-focused code review. OWASP-aware: injection, auth bypass, secrets handling, ' +
        'unsafe deserialization, supply-chain risks, attack-surface inflation.',
    prompt: 'You are a security reviewer. You read code looking for ways it can be exploited. ' +
...
export const SIMPLIFY_REVIEW_SUBAGENT = {
    id: 'simplify-review',
    name: 'Simplify Review',
    description: 'Anti-bloat / clarity review. Finds premature abstractions, dead code, defensive over-engineering, ' +
        'unnecessary wrappers, and complexity that exists only to satisfy hypothetical future requirements.',
    prompt: 'You are a simplicity reviewer. Your job is to find code that should not exist — code that ' +
...
export const TEST_COVERAGE_REVIEW_SUBAGENT = {
    id: 'test-coverage-review',
    name: 'Test Coverage Review',
    description: 'Identifies missing test cases, weak assertions, behavior that should be exercised end-to-end, ' +
        'and tests that pass by accident. Distinct from line-coverage metrics: focuses on behavioral coverage.',
    prompt: 'You are a test design reviewer. Your job is to find behavior that SHOULD be tested but ISN\'T, ' +
...
export const CORRECTNESS_REVIEW_SUBAGENT = {
    id: 'correctness-review',
    name: 'Correctness Review',
    description: 'Logic + concurrency + edge-case review. Finds race conditions, ordering assumptions, ' +
        'incorrect error handling, contract violations, and edge cases the implementation forgot.',
    prompt: 'You are a correctness reviewer. You read code looking for bugs — places where the implementation ' +
 ...
 * `slugifyName`.
 */
export function isValidSubagentId(id) {
    if (typeof id !== 'string')
        return false;
