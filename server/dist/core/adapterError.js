...
export class ContextLimitExceededError extends AdapterError {
    ...
    constructor(opts) {
        super({
            ...
            model: opts.model,
            summary: `Estimated context (${opts.approxTokens} tokens) exceeds adapter "${opts.adapter}" tier ` +
 ...
 */
function tryExtractFirstJson(s) {
    if (!s.trim())
        return null;
...
function detectTimeout(raw, adapter, model) {
...
export function detectClaudeError(raw, model) {
    ...
    if (/Rate limit reached/i.test(blob)) {
        ...
        const tpmMatch = blob.match(/(\d{1,3}(?:,\d{3})+|\d+)\s*tokens?\s*per\s*minute/i) ??
        ...
        return new AdapterError({
            ...
            actionable: 'Wait ~60s and retry, or pin to a different intelligence tier. ' +
...
export function detectGeminiError(raw, model) {
    ...
    if (msg) {
        if (/GEMINI_API_KEY\s+environment\s+variable/i.test(msg)) {
            return new AdapterError({
                ...
                actionable: 'Set GEMINI_API_KEY or GOOGLE_API_KEY in your environment ' +
        ...
        if (/API_KEY_INVALID|API key not valid/i.test(msg)) {
            return new AdapterError({
                ...
                actionable: 'Generate a new key at https://aistudio.google.com/apikey and update GEMINI_API_KEY.',
                raw,
            });
        }
        if (/RESOURCE_EXHAUSTED|Quota exceeded|"code"\s*:\s*429|429/i.test(msg)) {
            return new AdapterError({
                ...
                actionable: 'Wait per retryDelay, or upgrade Tier (Tier 1 paid = 300 RPM on flash).',
                raw,
            });
        }
...
export function detectCodexError(raw, model) {
        ...
        if (obj.type === 'turn.failed' || obj.type === 'error') {
            ...
            if (/429/.test(msg) || /Too Many Requests/i.test(msg) || /rate limit/i.test(msg)) {
                return new AdapterError({
                    ...
                    actionable: 'Wait per Retry-After header, or pin a different tier (intelligence: high uses gpt-5.5 separate pool).',
                    raw,
                });
            }
...
export function detectBinaryMissing(err, adapter, binary) {
