 ...
 */
export async function sweepIdleOnce(idleMs = DEFAULT_IDLE_MS) {
...
export async function sweepIdleNow(idleMs) {
    const r = await sweepBoth(idleMs, true);
    return r.closedThreads;
}
/** Combined sweep entry point — used by Phase 2.5. */
export async function sweepAll(idleMs = DEFAULT_IDLE_MS, force = false) {
    return sweepBoth(idleMs, force);
}
async function sweepBoth(idleMs, force) {
    const now = Date.now();
