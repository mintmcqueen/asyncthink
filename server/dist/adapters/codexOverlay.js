...
function overlayPathForThread(threadId) {
    return join(tmpdir(), 'asyncthink', 'codex-overlay', sanitizeThreadId(threadId));
}
function realCodexHomeFromEnv(env) {
 ...
 */
function extractServerSection(realConfig, name) {
    // Match `[mcp_servers.<name>]` (case-sensitive — TOML keys are case-sensitive).
    // Allow quoted or bare table keys.
    const safeName = name.replace(/[.\\+*?()|^$\[\]{}]/g, '\\$&');
    const headerRe = new RegExp(`^\\[mcp_servers\\.(?:"${safeName}"|${safeName})\\]\\s*$`, 'm');
    ...
    while ((m = nextHeaderRe.exec(after)) !== null) {
        ...
        const isChildOfThisServer = key.startsWith(`mcp_servers."${name}".`) ||
 ...
 */
export async function materializeCodexOverlay(args) {
    ...
    const header = '# Generated by AsyncThink — slim $CODEX_HOME overlay for MCP-allowlist enforcement.\n' +
 ...
 */
export async function cleanupCodexOverlay(threadId) {
 ...
 */
export function overlayExists(threadId) {
